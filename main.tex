...
\input{yaml-def}
 ...
 \lstdefinelanguage{my-kotlin}{
	...
	morestring=[b]",
	morestring=[s]{"""*}{*"""},
	ndkeywords={@Deprecated, @Serializable, @JvmField, @JvmName, @JvmOverloads, @JvmStatic, @JvmSynthetic, Array, Byte, List, Double, Float, Int, Integer, Iterable, Long, Runnable, Short, String, Unit, it},
	sensitive=true
}
...
Nowadays, many systems are adaptive and collective. Examples include drone swarms tasked with monitoring an area (as \Cref{fig:cas-examples-drone} shows),
wearable devices to manage crowd congestion during a public event (represented in \Cref{fig:cas-examples-crowd}), cars on streets connected to handle traffic \cite{DBLP:journals/sttt/NicolaJW20}.

\Cref{fig:cas-examples-crowd} is provided by \footnote{https://www.unibo.it/it/studiare/dottorati-master-specializzazioni-e-altra-formazione/insegnamenti/insegnamento/2023/483706}.
...
The meta-model of Alchemist, as \Cref{fig:alchemist-meta-model} and \Cref{fig:alchemist-reaction} show, is inspired by biochemistry and consists of various entities \footnote{https://alchemistsimulator.github.io/}:
\begin{itemize}
...
\Cref{fig:alchemist-nodes} shows some nodes in the Alchemist simulator interface.
...
In the discussion of this work, we will start with the design of the framework, as discussed in \Cref{ch:design}. 
...
\Cref{ch:implementation} delves into the implementation details, describing how various components of the framework were developed and the technologies employed. 
\Cref{ch:validation} outlines the testing of the platform, verification of integration with supported simulators, and presents a case study to validate adherence to the requirements defined in the analysis phase. 
Finally, in \Cref{ch:conclusion}, we analyze the work done and explore potential future developments.
...
The typical user interaction with the testbed is shown in \Cref{fig:high-level-execution}.
...
\Cref{lst:benchmark-configuration-sim2} depicts the configuration of two scenarios run by the Alchemist simulator.
...
\Cref{lst:model-impl} shows the implementation of the benchmark model.
...
As \Cref{lst:supportedSim} shows, the enum contains all the simulators supported by the framework. When a new simulator is added, it must be added to this enum.
...
The library \emph{"com.charleskorn.kaml:kaml"} is used to provide an easy and clean way to parse the input file as shown in \Cref{lst:yaml-parsing}.
...
\Cref{lst:lst:configfilehandler} shows the \emph{ConfigFileHandler} interface.
...
\paragraph*{Listener}
The listener is implemented as an interface with a \emph{read} method, as \Cref{lst:listener-interface} shows, which takes the path to the CSV file as input and returns a ScenarioOutput. 
...
The Alchemist implementation for the \emph{ClearCSV} method is depicted in \Cref{lst:alchemist-list}
...
An instance of \emph{ScenarioOutput} is shown in \Cref{lst:scenario-output-implementation}.
...
\Cref{lst:benchmark-output-impl} shows an instance of \emph{BenchmarkOutput}.
...
\Cref{lst:benchmark-result-impl} shows the implementation of the \emph{ScenarioResult} along an instance of it and the \emph{BenchmarkResult}.
...
As \Cref{lst:ctrl-lst:ctrl-executor} shows, the \emph{Controller} creates a map containing all the scenarios and their respective simulators, and then iterates over the map, executing each scenario.
...
\Cref{lst:ctrl-executor} shows the implementation of the \emph{createExecutor} method.
...
As shown in \Cref{lst:executor-interface}, the generation and the execution of the command are split into two methods, namely \emph{getCommand} and \emph{run}.
...
\Cref{lst:netlogo-executor} shows the implementation of the \emph{getCommand} method for the NetLogo simulator.
...
The project was developed following a customized version of Git Flow, depicted in \Cref{fig:git-flow}. 
...
\Cref{lst:custom-task} shows the implementation of the custom task to generate the JAR file.
...
\Cref{lst:parser-tests} shows the tests made on the parser.
...
The second execution was launched again through the framework, but this time with the graphical interface activated, as \Cref{fig:netlogo-sim} shows. 
...
This run is depicted in \Cref{fig:alchemist-sim}. The second execution was done headless and provided the same results as the first. This process was repeated for both incarnations supported by the framework, namely Protelis and Sapere.
...
We create a benchmark configuration file, \Cref{lst:cos-benchmark-conf}, which contains all the information about the benchmark to be executed.

\begin{lstlisting}[language=yaml, label={lst:cos-benchmark-conf}, caption={Case of study: benchmark configuration file.}]
    strategy:
      executionOrder:
...
This file, as \Cref{lst:cos-alchem-conf} shows, contains the environment configuration along with the export definition and the termination condition.
...
We proceed to implement a first version of the \emph{gradient} program as \Cref{lst:cos-alchem-solA} shows.
...
Now we define the processing function, as \Cref{lst:cos-postproc} depicts, which will be in charge of aggregating the data and returning the result.
...
In \Cref{fig:result-A} we can observe the results of the benchmark execution, along with the output data of the simulator.
...
To define the solution we take the Alchemist input file and change the time distribution of the reactions, as shown in \Cref{lst:cos-alchem-solB}.
...
\Cref{fig:result-B} shows the results of the benchmark execution.
