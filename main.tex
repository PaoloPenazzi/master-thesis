...
In the scientific research, new solutions are developed in order to improve what is defined as the state of art. 
...
Given the great number of simulators already available, the framework must flexible and extensible.
...
\ac{CAS} are a complex type of distributed network which are composed of a many heterogeneous entities, each with its own capabilities and goals. 
...
This trend has led to a change in the way we compute data. 
In fact, the focus is no longer on a single machine performing heavy computation, but rather on a distributed network of devices communicating and collaborating to achieve a result.
Computation is thus divided and distributed across various devices in the network, introducing an additional level of complexity in programming these systems.
In fact, developers must now consider issues such as communication between devices, concurrency, or failures. 
...
Alchemist is an open-source tool for simulating complex distributed systems. It is termed a meta-simulator because it is based on generic abstractions.
...
Currently, it only includes the execution order of the scenarios, a mandatory parameter which defines the sequence in which the scenarios will be executed.
...
\Cref{lst:benchmark-configuration} is an input file example, which clarifies the structure of the benchmark configuration file.

\begin{lstlisting}[language=yaml, label={lst:benchmark-configuration} ,caption={Benchmark configuration file structure}]
strategy:
  executionOrder:
...
A post-processing function must then be implemented, taking this \emph{BenchmarkOutput} as input and returning a \emph{BenchmarkResult}. 
This solution divides the process into two phases, where the script execution is also performed. TODO
...
Since the framework relies on external simulators to run a benchmark, it is necessary for the user to have these simulators installed on their machine. 
...
This allows users to assess a solution behavior in different scenarios, utilizing different simulators, and facilitating the comparison of the obtained results.
...
\begin{itemize}
  ...
  The comparison of new and existing solutions on well-known problems is a fundamental aspect when developing innovative solutions.
