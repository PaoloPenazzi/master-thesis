...
A \textit{testbed} is a platform for for experimentation and testing. It provides a rigorous, transparent and replicable environment for testing.
...
A \textit{benchmark} is a reference against which a system can be compared.
...
This allows to understand whether a newly developed solutions is better than an existing one in a certain scenario.

Unfortunately, there is a lack of platform to perform tests and benchmarks to compare to.

This work targets a specifing category of systems, called \ac{CAS}.

% Definition of CAS
\ac{CAS} are composed of a large number of entities that interact with each other and with the environment to achieve a common goal.
...
In nature, a colony of ants is a \ac{CAS}, where each ant is an autonomous entity that interacts with other ants and with the environment to achieve the common goal of the colony. Other examples are bird flocks, fish shoals, animal herds, human crowd etc.
...
% Adaptivity
\begin{itemize}
    \item Adaptation to the environment is not carried out by a single component, but rather emerges by the interaction of several entities. \cite{3}
\end{itemize}
