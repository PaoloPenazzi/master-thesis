...
\Cref{fig:alchemist-interface} shows some nodes in the Alchemist simulator interface.
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=\textwidth]{figures/NetLogo-interface.png}
...
The framework must be designed to allow the user to define a benchmark, execute it, and compare the result to state-of-the-art solutions.
To assess the system's behavior, the testbed relies on existing simulators in the market. 
Given the large number of these simulators, the tool must be flexible and extensible to include support for new simulators later on without compromising the functionality of those already integrated or the framework as a whole \cite{DBLP:conf/mascots/Dujmovic99}.
...
With this work, the aim is to establish the foundations for a standard way to test and compare various solutions in the \ac*{CAS} domain.
...
This run is depicted in \Cref{fig:alchemist-sim}.
...
In \Cref{fig:result-A} we can observe the result of the benchmark execution, along with the output data of the simulator.
...
\Cref{fig:result-B} shows the result of the benchmark execution.
