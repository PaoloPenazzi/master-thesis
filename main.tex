...
\begin{abstract}
  ...
  In various fields of computer science, tools have been developed to benchmark new solutions.
  ...
  The primary objective of this thesis is to craft a prototype for a benchmarking platform tailored to Collective Adaptive Systems (CAS). 
  By capitalizing on existing simulators available in the market, the aim is to establish a comprehensive framework for testing, validating, and comparing these dynamic systems. 
  The presented platform is designed to empower users in defining benchmarks, executing them, and extracting results of interest - all while preserving flexibility and extensibility. 
  This inherent adaptability allows for the incorporation of additional simulators into the expanding testbed.

  An experiment has been executed to validate the framework's anticipated functionalities and scrutinize its strengths and weaknesses. 
  This undertaking serves the purpose of identifying areas for future improvement within the tool.
\end{abstract}
...
Each simulator has its own method of providing simulation results. Some return a CSV file, others a text file, and some even utilize snapshots. 
...
Upon execution completion, the framework reads the results from a predefined JSON file: result.json. 
This file must contain data in the form of Scenario Result, which will be described shortly.

In the second case, the Listener interface is employed. It takes the file outputted by the simulator and transforms it into an internal framework data structure: BenchmarkOutput. 
A post-processing function must then be implemented, taking this BenchmarkOutput as input and returning a BenchmarkResult. 
In practice, this solution divides the process into two phases, where the script execution is also performed.
This allows defining a method to read the output of a simulator and using it in combination with various processing functions to obtain results of different natures.
...
The work outlined in this thesis has culminated in the development of a testbed prototype designed for benchmarking collective adaptive systems. 
The primary aim of the framework is to offer a tool that integrates the most widely used simulators within the realm of collective adaptive systems. 
It empowers users to assess solution behavior in diverse scenarios, utilizing different simulators, and facilitating the comparison of obtained results.

To realize this objective, the framework was meticulously crafted to be flexible and extensible. 
Users can effortlessly express benchmark configurations in a straightforward and intuitive manner, and seamlessly integrate new simulators without disrupting the existing structure.

A pivotal aspect of the framework lies in its abstract design. 
Each system component boasts a general behavior independent of the simulator, yet remains incomplete. 
This deliberate design allows users to extend the framework by incorporating specific logic for new simulators. 
For users less inclined to introduce support for new simulators, the framework still serves as a robust tool for thoroughly testing solution behavior in specific scenarios.

Furthermore, the framework promotes community collaboration, enabling individuals to define benchmarks and share results. 
This communal approach allows users to incorporate their solutions into pre-configured benchmarks, fostering result comparisons.

In our assessment, the framework serves as a promising foundation for the development of a comprehensive tool for testing collective adaptive systems. 
It effectively tackles key challenges in open benchmarking platform development, providing a sturdy base for future enhancements.
